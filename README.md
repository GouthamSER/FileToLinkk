...
**File 2 Link™** is a powerful Telegram bot that transforms Telegram files into high-speed direct links, perfect for both streaming and rapid downloading. Share files via HTTP(S) links without needing to download them from the Telegram client first.
...
| `FQDN` | Domain/IP address | `f2l.file2link.com` |
...
docker run -d --name file2link -p 8080:8080 file2link
```
...
[![Deploy to Koyeb](https://www.koyeb.com/static/images/deploy/button.svg)](https://app.koyeb.com/deploy?type=docker&image=docker.io/GouthamSER/file2link:latest&name=file2link&ports=8080;http;/&env[API_ID]=&env[API_HASH]=&env[BOT_TOKEN]=&env[BIN_CHANNEL]=&env[OWNER_ID]=&env[DATABASE_URL]=&env[FQDN]=)
...
2. Choose **Existing Image**: `GouthamSER/file2link:latest`
...
2. Choose **Docker Image**: `GouthamSER/file2link:latest`
3. Add your environment variables
4. Click **Deploy**
...
-   A **subdomain** (e.g., `f2l.file2link.com`) set up in **Cloudflare**.
...
```bash
sudo mkdir -p /etc/ssl/cloudflare/f2l.file2link.com
# Move/copy your cert.pem and key.key files into this directory
sudo chmod 600 /etc/ssl/cloudflare/f2l.file2link.com/key.key
sudo chmod 644 /etc/ssl/cloudflare/f2l.file2link.com/cert.pem
```
...
Create a new file at `/etc/nginx/sites-available/f2l.file2link.conf` and paste the following, replacing `f2l.file2link.com` and `8080` with your values.

```nginx
server {
    listen 443 ssl;
    listen [::]:443 ssl;
    server_name f2l.file2link.com;

    # SSL Configuration
    ssl_certificate     /etc/ssl/cloudflare/f2l.file2link.com/cert.pem;
    ssl_certificate_key /etc/ssl/cloudflare/f2l.file2link.com/key.key;
...
```bash
sudo ln -s /etc/nginx/sites-available/f2l.file2link.conf /etc/nginx/sites-enabled/
sudo nginx -t
...
-   **With a Domain**: Use your subdomain (e.g., `f2l.file2link.com`).
